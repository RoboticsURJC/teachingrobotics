...
```
* `GUI.getMap(url)` - Returns a numpy array with the image data in a 3 dimensional array (R, G, B, A). The URL of the Vacuum Cleaner Loc map is '/RoboticsAcademy/exercises/static/exercises/vacuum_cleaner_loc/resources/mapgrannyannie.png', so the instruction to get the map is
```
array = GUI.getMap('/RoboticsAcademy/exercises/static/exercises/vacuum_cleaner_loc/resources/images/mapgrannyannie.png')
```
